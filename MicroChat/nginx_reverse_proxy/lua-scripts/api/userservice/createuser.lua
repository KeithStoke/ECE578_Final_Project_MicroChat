...
function _M.CreateUser()
	...
	ngx.req.read_body()
        ...
        if (_StrIsEmpty(post.username) or _StrIsEmpty(post.name) or _StrIsEmpty(post.password) ) then
	...
	ngx.say("Inside Nginx Lua script: Creating a new user...")
	
	local client = GenericObjectPool:connection(UserServiceClient, "user-service", 9090)
	local status, ret = pcall(client.CreateUser, client, post.username, post.name, post.password)
	...
	if not status then
    		...
    		ngx.header.content_type = "text/plain"
			ngx.say("User creation successful: ", ret)
