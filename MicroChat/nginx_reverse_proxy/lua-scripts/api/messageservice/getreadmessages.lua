...
function _M.GetReadMessages()
	...
	ngx.say("Inside Nginx Lua script: Getting read messages for ", post.username)
	
	local client = GenericObjectPool:connection(MessageServiceClient, "message-service", 9091)
	local status, ret = pcall(client.GetReadMessages, client, post.username)
